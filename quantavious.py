...
FORECAST_HORIZON_HOURS = 32  # 5 days * ~6.5 hours/day (9:30 AM–4:00 PM)
LSTM_WINDOW_DAILY = 180
LSTM_WINDOW_HOURLY = 325  # ~50 days * 6.5 hours/day
LSTM_EPOCHS = 15
LSTM_BATCH_SIZE = 16
...
def upload_to_spaces(file_path, key):
    try:
        ...
        print(f"Uploaded {file_path} to Spaces as {key}")
...
def get_retail_institutional_metrics(symbol, start_date, end_date, timeframe="day"):
    try:
        ...
        print(f"Error getting retail/inst metrics for {symbol} ({timeframe}): {e}")
        return None
...
def compute_correlations(output_dir=DRIVE_SAVE_DIR, forecast_horizon=FORECAST_HORIZON_DAYS, timeframe="day"):
    ...
    # Aggregate correlations (mean across stocks)
    if corr_data:
        ...
        print(f"No correlation data for {timeframe}")
        return None
...
def process_ticker(symbol, start_date, end_date, forecast_horizon, window, timeframe="day"):
    try:
        ...
        if retail_inst_df is None:
            print(f"Retail/inst metrics failed for {symbol}")
            return None
...
def compute_aggregates(output_dir=DRIVE_SAVE_DIR, forecast_horizon=FORECAST_HORIZON_DAYS, timeframe="day"):
    ...
    print(f"Aggregated averages saved to {aggregate_path}")
